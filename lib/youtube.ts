...
export async function fetchYTPosts(includeShorts = true): Promise<YTPost[]> {
  if (!APIFY_TOKEN || !YT_CHANNEL_URL) return []

  const items = await runApifyActor({
    startUrls: [{ url: YT_CHANNEL_URL }],
    maxResults: 25,
    maxResultsShorts: includeShorts ? 25 : 0,
    maxResultsStreams: 0,
  ...
  return items
    ...
    .map(item => {
      ...
      // Detect shorts: type field or URL contains /shorts/
